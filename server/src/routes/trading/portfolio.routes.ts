...
import { computeSummary, computeHistory, TradeInput, HistoryTradeInput } from '../../services/trading/portfolioService.js';
import { computeTaxSummary, replayLotsAndGains, detectWashSales, estimateTax, type TaxTradeInput } from '../../services/trading/taxService.js';
import { getCurrentPrices } from '../../services/market/priceService.js';
import { authenticate } from '../../middleware/auth.js';
import { getSector } from '../../services/data/tickerMetadata.js';
...
router.get('/:id/tax', async (req: Request, res: Response) => {
  try {
    ...
    const taxTrades: TaxTradeInput[] = trades.map((t) => ({
      ticker: t.ticker,
    ...
    const summary = computeTaxSummary(taxTrades);
    res.json({ portfolioId: id, ...summary });
  } catch (err) {
    console.error(`Tax error for ${req.params.id}:`, err);
    res.status(500).json({ error: 'Failed to compute tax summary' });
  }
});

// ── GET /api/portfolios/:id/tax-preview ─────────────────
// Preview tax impact of a hypothetical SELL without executing it.
// Query: ?ticker=AAPL&quantity=10&price=150
router.get('/:id/tax-preview', async (req: Request, res: Response) => {
  try {
    const userId = req.user!.userId;
    const id = Number(req.params.id);
    ...
    if (!ticker || !quantity || !price) {
      res.status(400).json({ error: 'ticker, quantity, and price query params required' });
      return;
    }
    ...
    // Add hypothetical sell
    const hypotheticalSell: TaxTradeInput = {
      ticker,
      side: 'SELL',
      quantity,
      price,
    ...
    const withSell = [...taxTrades, hypotheticalSell];
    const { gains } = replayLotsAndGains(withSell);

    // Only the gains from the hypothetical sell (last ones added)
    const originalGainCount = replayLotsAndGains(taxTrades).gains.length;
    const newGains = gains.slice(originalGainCount);

    const washSales = detectWashSales(withSell, newGains);
    ...
    console.error(`Tax preview error for ${req.params.id}:`, err);
