...
export default function TradeForm({
  ...
  const handleNext = () => {
    if (activeStep === 0) {
      ...
      setActiveStep(2);
      // Fetch tax preview for SELL orders
      if (side === 'SELL' && orderType === 'MARKET' && livePrice) {
        setTaxPreview(null);
        apiClient
          .get(`/portfolios/${portfolioId}/tax-preview`, {
            params: {
      ...
      <Collapse in={activeStep === 2} timeout={350} unmountOnExit>
        <Box sx={{ maxWidth: 420, mx: 'auto' }}>
          ...
          >
            ...
            {/* Tax Preview for SELL orders */}
            {side === 'SELL' && taxPreview && (
              <Box
                sx={{
                  mt: 2,
                  p: 1.5,
                  borderRadius: 1,
                  bgcolor: 'rgba(255,171,0,0.06)',
                  border: '1px solid rgba(255,171,0,0.15)',
                }}
              >
                <Typography variant="caption" sx={{ textTransform: 'uppercase', letterSpacing: '0.06em', color: '#ffab00', fontWeight: 700, mb: 0.5, display: 'block' }}>
                  Tax Impact
                </Typography>
                {taxPreview.shortTermGain !== 0 && (
                  <Typography variant="body2" color="text.secondary">
                    Short-term: <span style={{ color: taxPreview.shortTermGain >= 0 ? '#00C805' : '#ff5252', fontWeight: 600 }}>${fmt(taxPreview.shortTermGain)}</span>
                  </Typography>
                )}
                {taxPreview.longTermGain !== 0 && (
                  <Typography variant="body2" color="text.secondary">
                    Long-term: <span style={{ color: taxPreview.longTermGain >= 0 ? '#00C805' : '#ff5252', fontWeight: 600 }}>${fmt(taxPreview.longTermGain)}</span>
                  </Typography>
                )}
                <Typography variant="body2" color="text.secondary">
                  Est. tax: <span style={{ color: '#ff5252', fontWeight: 600 }}>${fmt(taxPreview.estimatedTax)}</span>
                </Typography>
                {taxPreview.washSaleWarning && (
                  <Typography variant="caption" sx={{ color: '#ffab00', mt: 0.5, display: 'block' }}>
                    Wash sale warning: loss may be disallowed
                  </Typography>
                )}
              </Box>
            )}
